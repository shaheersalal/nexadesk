  ...
  return (
    <div className="text-center py-10">
      ...
      {actionLabel && actionTo && (
        <Link to={actionTo} className="inline-block text-xs font-semibold text-accent hover:text-accent-dark mt-3">
          {actionLabel} →
        </Link>
      )}
      {actionLabel && onAction && !actionTo && (
        <button onClick={onAction} className="text-xs font-semibold text-accent hover:text-accent-dark mt-3">
          {actionLabel} →
        </button>
      )}
    </div>

...
### [ ] C1 — Unrecognised inbound number is routed to an arbitrary tenant
...
### [ ] C2 — Admin token emailed in plaintext as a URL parameter
...
### [ ] C3 — Twilio webhook signature validation fails open
...
### [ ] H1 — Two blocking network calls on every authenticated request
...
### [ ] H2 — Outbound call audio uses the wrong stream identifier
...
### [ ] H3 — Public demo chat is an unmetered LLM cost sink
...
### [ ] H4 — Invalid API key returns HTTP 500 instead of 401
...
### [ ] H5 — Fire-and-forget webhook tasks can be garbage collected
...
### [ ] H6 — Webhook retry loop runs once per uvicorn worker
`app/main.py:91-101`
...
### [ ] H7 — Tenant-supplied webhook URLs enable SSRF
...
### [ ] H8 — Any company member can mint full-scope API keys
...
### [ ] H9 — CRM OAuth tokens stored in plaintext
...
### [ ] M1 — Audio buffer is 160 ms, not the documented 1 second
...
### [ ] M2 — STT uses the batch endpoint, not streaming
...
### [ ] M3 — Synchronous translation blocks the event loop
...
### [ ] M4 — Synchronous company fetch on every voice turn
...
### [ ] M5 — Client IP is attacker-controlled, defeating the throttle
...
### [ ] M7 — Demo email crashes when a discount has no price
...
### [ ] M8 — Unescaped user input interpolated into outbound email HTML
...
### [ ] M9 — `bootstrap_company` can silently relocate an existing user
...
### [ ] M10 — `last_used` write on every API request
...
### [ ] M11 — Untyped JSON-RPC arguments reach `min()` unvalidated
...
### [ ] M12 — Unguarded `result.data[0]` after insert
...
### [ ] M13 — Default `LLM_MODEL` is incompatible with the client
`app/config.py:17`
...
### [ ] M14 — SPA catch-all swallows unknown API paths
`app/main.py:163-171`
...
### [ ] L1 — CI "Verify imports" step is a no-op
...
### [ ] L3 — 115 MB of orphaned model weights
...
### [ ] L4 — Uploads written to ephemeral local disk
`app/main.py:87`

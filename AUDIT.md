...
dashboard, SQL schema and CI. Runtime verification is limited â€” there is no test
...
Legend: `[ ]` open Â· `[x]` fixed Â· severity **C**ritical / **H**igh / **M**edium / **L**ow
...
H2, M1, M2 are the voice pipeline findings — fixed together in the streaming
rewrite rather than as point patches. M15 is environment configuration, covered
in DEPLOYMENT.md. L2 is the test suite. L5 is the demo consolidation. L6 and L7
are recorded architectural notes, not scheduled work.
...
provisioned â€” which is exactly why they must be closed before step 8.

---

## CRITICAL

### [x] C1 â€” Unrecognised inbound number is routed to an arbitrary tenant
`app/voice/router.py:242-250`

`_resolve_company_id` looks up the company by the dialled number, and when no
row matches it falls back to `SELECT id FROM companies LIMIT 1` â€” an arbitrary
tenant.
...
### [x] C2 â€” Admin token emailed in plaintext as a URL parameter
...
an inbox, in Resend's delivery logs, and â€” once clicked â€” in server access logs,
...
### [x] C3 â€” Twilio webhook signature validation fails open
...
token is blank â€” same treatment `APP_SECRET_KEY` already gets at `main.py:76`.

---

## HIGH

### [x] H1 â€” Two blocking network calls on every authenticated request
`app/dependencies.py:102-128`

`get_current_user` constructs a **new** synchronous Supabase client per request
(line 111) and calls `client.auth.get_user(token)` â€” a network round-trip â€” then
...
dependency) against the project JWT secret â€” zero network calls â€” and cache
`company_id` in Redis keyed by user id.

### [ ] H2 â€” Outbound call audio uses the wrong stream identifier
...
nothing â€” the AI's greeting and every reply are dropped. LIKELY rather than
...
### [x] H4 â€” Invalid API key returns HTTP 500 instead of 401
...
`.single()` pattern as fixed elsewhere â€” these two are regressions of a
previously-known bug.

**Fix:** use `.maybe_single()` or `.limit(1)` and return 401 explicitly.

### [x] H5 â€” Fire-and-forget webhook tasks can be garbage collected
...
dropped â€” no delivery, no log row update, no error. The customer's CRM
desynchronises and nothing indicates why.

**Fix:** keep a module-level `set` of pending tasks and discard on completion.

### [x] H6 â€” Webhook retry loop runs once per uvicorn worker
`app/main.py:91-101`
...
retries and deliver each one â€” the customer's endpoint receives up to 4Ã—
...
### [x] H7 â€” Tenant-supplied webhook URLs enable SSRF
...
response status into `webhook_logs`, which the tenant can read back â€” a blind
...
### [x] H8 â€” Any company member can mint full-scope API keys
...
### [x] H9 â€” CRM OAuth tokens stored in plaintext
...
**Failure scenario:** Any read access to that table â€” a leaked service key, a SQL
injection elsewhere, a Supabase backup, or the service-role key that every
backend route already uses â€” yields live OAuth tokens granting write access to
...
### [ ] M1 â€” Audio buffer is 160 ms, not the documented 1 second
...
roughly 6Ã— per second per call, each transcribing a fragment far too short to
contain a complete phrase. Drives both cost and the latency problem in M2.

### [ ] M2 â€” STT uses the batch endpoint, not streaming
...
### [x] M3 â€” Synchronous translation blocks the event loop
...
### [x] M4 â€” Synchronous company fetch on every voice turn
...
### [x] M5 â€” Client IP is attacker-controlled, defeating the throttle
...
### [x] M7 â€” Demo email crashes when a discount has no price
...
**Failure scenario:** A client posts `discount_pct=20` with null prices â€”
`TypeError: unsupported format string passed to NoneType` â€” unhandled, HTTP 500,
and the demo request is lost after already being inserted.

### [x] M8 â€” Unescaped user input interpolated into outbound email HTML
...
### [x] M9 â€” `bootstrap_company` can silently relocate an existing user
...
### [x] M10 â€” `last_used` write on every API request
...
### [x] M11 â€” Untyped JSON-RPC arguments reach `min()` unvalidated
`app/mcp/server.py:119, 145, 154`

`min(args.get("limit", 20), 100)` raises `TypeError` when a client sends
`"limit": "20"` â€” legal JSON-RPC, since the schema is advertised but never
...
### [x] M12 â€” Unguarded `result.data[0]` after insert
`app/v1/router.py:90`, `app/mcp/server.py:133`, `app/auth/middleware.py:47`,
`app/integrations/api_keys_router.py:60`

`IndexError` â†’ 500 whenever an insert returns no representation (RLS rejection,
trigger, or a `returning=minimal` client default).

### [x] M13 â€” Default `LLM_MODEL` is incompatible with the client
`app/config.py:17`
...
### [x] M14 â€” SPA catch-all swallows unknown API paths
`app/main.py:163-171`
...
### [ ] M15 â€” Production guards are inert in the committed environment
...
### [x] L4 â€” Uploads written to ephemeral local disk
`app/main.py:87`

`uploads/` does not survive a Railway redeploy. `SUPABASE_STORAGE_BUCKET` is
configured but unused.

### [ ] L5 â€” Demo app duplicated and diverged
...
### [ ] L6 â€” Dashboard ships an in-browser Whisper runtime
...
### [ ] L7 â€” RLS is bypassed by the backend
`app/dependencies.py:19-22`

The SQL schema's row-level security is thorough and correct â€” but every backend
...
- **`call_session.py` is fully Redis-backed** with a 1 h TTL. No in-memory call
  state â€” horizontal scaling on Railway is safe.
...
- **`_RETRY_DELAYS` indexing in `events.py:67-69` is safe** â€” the
...
- **The pricing discount ladder** is server-authoritative and prompt-injection
  resistant by design (`app/pricing/router.py:47-68`) â€” the LLM never chooses the
  percentage.

---

## Deferred â€” cost and architecture

Not fixed in this pass; recorded for a decision after deploy.

- **TTS is ~85% of per-minute call cost.** ElevenLabs at ~$0.12â€“0.22 per 1k chars
...
- **Overage pricing is below cost after discount.** `PER_MINUTE_OVERAGE = $0.18`
  against ~$0.10â€“0.17 actual; the 20% discount ladder puts it at $0.144 â€” a loss
  on ElevenLabs' lower tiers.
- **Sequential turn pipeline** costs ~2.5â€“4.5 s of dead air. Addressed by the
